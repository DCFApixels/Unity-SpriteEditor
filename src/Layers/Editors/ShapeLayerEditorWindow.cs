...
namespace DCFApixels.SpriteEditor
{
    public sealed class ShapeLayerEditorWindow : LayerEditorWindowBase
    {
        ...
        internal static void BuildFields(VisualElement root, ShapeLayerBehaviour layer, TextureCompositor compositor,
            Action<string, Action> apply, SpriteEditorUI.ValueBindings bindings)
        {
            var kind = SpriteEditorUI.ConfigureField(new EnumField("Shape", layer.kind));
            ...
            {
                var field = SpriteEditorUI.ConfigureField(SpriteEditorColorInputs.Bind(new ColorField(label), bindings, get));
                field.RegisterValueChangedCallback(evt => apply("Change Shape " + label, () => set(evt.newValue)));
                root.Add(field);
            }
            ...
            var width = SpriteEditorUI.ConfigureField(new FloatField("Stroke Width (px)"));
            ...
            root.Add(roundness);
            var sides = SpriteEditorUI.ConfigureField(new SliderInt("Sides / Points", 3, 32) { showInputField = true });
            ...
            bindings.Add(() =>
            {
                ...
                roundness.EnableInClassList("sprite-editor-hidden", layer.kind != ShapeLayerBehaviour.ShapeKind.Rectangle);
                sides.EnableInClassList("sprite-editor-hidden", layer.kind != ShapeLayerBehaviour.ShapeKind.Polygon && layer.kind != ShapeLayerBehaviour.ShapeKind.Star);
                inner.EnableInClassList("sprite-editor-hidden", layer.kind != ShapeLayerBehaviour.ShapeKind.Star);
            });
        }
    }
}
