...
| Эффекты | Outline, SDF, Normal Map, Gaussian Blur, Motion Blur, режимы смешивания и встроенные Shader FX с редактируемым HLSL. |
...
| Rectangle Select | `M` | Выделение прямоугольной области перетаскиванием. |
| Polygonal Lasso | `L` | Выделение многоугольника последовательными кликами. |
...
<details>
<summary>Пользовательские настройки, окна Properties и сброс рабочего пространства</summary>

Каждый вызов **⋮ → Properties** у слоя открывает отдельное окно. **FX** находится в том же меню строки.

**Меню ⋮ вкладки окна → User Settings…** открывает выбор двух цветов шахматки, размера клетки (1–128 пикселей интерфейса, по умолчанию 16) и цвета подсветки
невалидных пикселей. Цвета сразу обновляются во всех открытых превью и сохраняются для пользователя,
не изменяя документы, экспорт и Undo. **Reset Preview Appearance** возвращает стандартные цвета и размер клетки.

**Меню ⋮ вкладки окна → Reset Sprite Editor Settings…** сбрасывает размеры панелей, прокрутку,
выделение, раскрытие разделов, инструменты превью, общие параметры рисования, цвета превью, каналы и Live Quality.
...
**Типы слоёв:** File · Drawing · Color Fill · Gradient · Noise · Outline · SDF · Normal Map · Gaussian Blur · Motion Blur · Shader Processor · Group.
...
Добавь **Noise Layer** для генерации OpenSimplex2, OpenSimplex2S, Perlin, Value, Value Cubic или Cellular на GPU.
Перетаскивай подписи числовых полей **Scale**, **Offset** и других параметров либо используй слайдеры:
превью обновляется во время редактирования, без перестройки панели и ожидания отпускания мыши.
Seed, фракталы и Domain Warp сохраняются как параметры, а не полноразмерные снимки пикселей.

**Scale** задаёт масштаб по короткой стороне холста: больше значение — мельче детали. **Offset** использует
координаты пространства шума. **Color Values** выводит градации серого для просмотра; **Linear Data** оставляет
сырые значения 0–1 для масок, карт высот и упаковки каналов. В RGB одно и то же значение, альфа равна 1;
назначение каналов выполняется через Swizzle и смешивание. Трансформ, обтравка и Shader FX доступны как обычно.

Генерация прямая, в запрошенном разрешении. Обычное превью сохраняет существующий лимит 512 px,
полноразмерный экспорт использует те же координаты и seed. Нового двухэтапного уточнения разрешения
и фоновой анимации нет. Tiled повторяет результат, но сам шум пока не является бесшовным.
Используется FastNoiseLite HLSL; [лицензия стороннего кода](ThirdPartyNotices.md).
...
| **Лист +** | Создать пустой Drawing-слой над активным. | Слить перетаскиваемые слои/группы в новый Drawing-слой, сохранив оригиналы. |
...
Команды в **⋮** и меню по правому клику применяются ко всему выделению. Команды для групп
действуют на выделенные группы; **Properties** и **FX** открывают отдельное окно для каждого
подходящего слоя. При перемещении, дублировании, конвертации и удалении группа включает потомков один раз.

**⋮ → Duplicate** копирует выделенные слои и группы целиком. Копии появляются над оригиналами.
...
### Выделение области и буфер обмена

**Rectangle Select** (`M`) и **Polygonal Lasso** (`L`) используют общее выделение в координатах холста.
У лассо задавай вершины кликами; завершай через `Enter`, двойной клик, клик по первой вершине или **Close**.
`Backspace` / ПКМ удаляет последнюю вершину, `Esc` отменяет незавершённую фигуру.
В шапке доступны Replace, Add, Subtract и Intersect; `Shift` добавляет область, `Alt` вычитает,
`Shift+Alt` оставляет пересечение.

- `Ctrl`-клик по миниатюре слоя (стрелке группы) выделяет его альфу с трансформом, FX, Swizzle
  и обтравкой. Общая видимость и прозрачность самого слоя игнорируются; у группы учитываются
  настройки дочерних слоёв. `Ctrl`-клик по остальной строке по-прежнему управляет мультивыделением слоёв.
- Область ограничивает кисть, карандаш, стирание и заливку и сохраняется при смене слоя или инструмента.
  `Ctrl+A` выделяет всё, `Ctrl+D` снимает выделение, `Ctrl+Shift+I` инвертирует его.
  Пустое активное выделение полностью запрещает рисование.
- `Ctrl+C` копирует область активного слоя/группы с его отрисованными настройками;
  `Ctrl+Shift+C` — видимую композицию. Без выделенной области копируется весь холст.
- `Ctrl+V` добавляет независимый Drawing-слой наверх корневого списка, с HDR-пикселями и сброшенным
  трансформом. На холсте того же размера сохраняется позиция; на другом область центрируется
  и обрезается по его границам. Вставка поддерживает Undo/Redo и не накладывает текущую маску повторно.

Буфер внутренний для Sprite Editor: он общий между окнами до перезагрузки скриптов или выхода из Unity,
но не связан с системным буфером изображений. Копирование не учитывает EV, каналы и Live Quality превью.
Выделение — временное состояние окна, не настройка ассета и не шаг Undo: оно сбрасывается при смене
документа, размеров холста, закрытии окна или перезагрузке скриптов и не ограничивает штрихи через API агентов.
Лимит — 16 777 216 пикселей холста. В Tiled выделение переносится через края; сложный контур отображается
с упрощением, но маска рисования остаётся полноразмерной. Лассо ограничено 256 вершинами и 16 повторами по высоте.

### Обтравочные маски слоёв

Включи **Clipping Mask** в контекстном меню слоя или нажми `Alt` на границе над его базой.
Команда меню применяется ко всем выделенным слоям, а клик по границе переключает только верхнего соседа.
Обтравочные слои отмечены `↳`. Несколько таких слоёв подряд используют общую базу — первый
слой без обтравки под ними, внутри той же группы. Перестановка слоёв меняет базу.

Цепочка сохраняет альфу базы, включая мягкие края; прозрачность базы применяется к общему результату
один раз. Верхние слои сохраняют цвет, наложение, прозрачность, трансформ, FX и Swizzle,
но не расширяют покрытие базы. **Overwrite** внутри обтравки заменяет цвет в пределах покрытия
источника, не стирая альфу базы. Скрытая, прозрачная или отсутствующая база скрывает цепочку.
Группа, участвующая в обтравке как база или верхний слой, временно изолируется:
для выбранного Pass Through используется Normal до выхода из цепочки.

Обтравка не меняет исходные пиксели и поддерживает Undo/Redo, копирование, конвертацию в Drawing,
API агентов и нативные флаги PSD. Цели Outline/SDF учитывают обтравленное покрытие.
При слиянии маска запекается; если база не выделена, от неё берётся только маска, без цвета.
Зависимое от фона смешивание может измениться, как и при других частичных слияниях.
...
`Color Fill n`, `Gradient n`, `Noise n`, `Outline n`, `SDF n`, `Normal Map n`, `Gaussian Blur n`, `Motion Blur n` и `Group n`.
...
Слои эффектов (Outline, SDF, Normal Map, Gaussian Blur и Motion Blur) обрабатывают скрытые источники в режимах Previous
и Specific. Глазик источника управляет его собственной видимостью в композиции, но не доступностью
для эффекта. Скрытая группа передаёт содержимое видимых дочерних слоёв; отдельно скрытые дети
по-прежнему исключены. Прозрачность источника и поведение обтравочных масок не меняются.

Для группы используется grayscale-покрытие её собственной изолированной композиции;
бленды и прозрачность видимых потомков учитываются без внешнего фона.
...
Добавь **Gaussian Blur Layer** и выбери **Previous** либо назначь слой/группу через **Specific**.
Скрытые источники доступны эффекту. Группа передаёт собственную изолированную композицию,
но в общем рендере может оставаться сквозной. **Radius** — радиус ядра в пикселях исходного
холста (0–256, по умолчанию 8); ноль отключает размытие. **Edges**: Transparent (по умолчанию),
Clamp, Repeat и Mirror. Для бесшовных текстур выбери Repeat; режим Tiled сам эту настройку не меняет.

Цвет и альфа размываются вместе в линейном пространстве с учётом прозрачности. Для сохранения
интенсивностей выше 1 установи Color Range эффекта в HDR. Во время рисования и изменения
настроек большие ядра рассчитываются приближённо на уменьшенном изображении; после завершения
редактирования превью уточняется автоматически. Сохранение, экспорт и конвертация используют
полноценный алгоритм; в PSD результат становится растровым слоем.
Основное окно использует общий ограниченный кеш эффектов и источников групп, отдельно от Undo.
Подробнее: [рендер и кеш](Documentation~/GaussianBlur.md), [параметры API](Documentation~/AgentAPI.md#gaussian-blur-settings).

### Motion Blur

Добавь **Motion Blur** и выбери источник **Previous** или **Specific**, включая скрытый слой
или изолированную группу. В режиме **Linear** доступны Distance (0–512 пикселей холста) и Angle;
в **Circular** — Arc (0–360 градусов) и нормализованный Center. `(0.5, 0.5)` — центр холста.
Circular размывает по дуге вращения, а не приближает изображение к центру.

**Strength** (0–400%, по умолчанию 100%) до 100% смешивает результат с исходником, а выше 100%
делает полупрозрачный след плотнее, не меняя длину и яркость цвета. Полностью непрозрачные
участки выше 100% не меняются. При 0% возвращается исходное изображение.

**Direction**: Centered — в обе стороны, Forward — по линейному углу либо против часовой стрелки
вокруг центра, Backward — в обратную сторону. **Edges**: Transparent, Clamp, Repeat и Mirror.
Как у Gaussian Blur, учитываются прозрачность и HDR, используется общий кеш эффектов,
а после редактирования основное превью уточняется. У больших круговых дуг число выборок ограничено:
на тонких деталях могут быть заметны отдельные следы. В PSD результат растрируется.
Подробнее: [рендер](Documentation~/MotionBlur.md), [параметры API](Documentation~/AgentAPI.md#motion-blur-settings).

### Normal Map

**Settings → Simple / Advanced** переключает только набор видимых полей в Layer Settings и Properties.
По умолчанию выбран Simple; Advanced показывает всё, с разделами Source, Surface, Height Levels,
Texture Detail и Output. Выбор запоминается на сессию Editor. Скрытые значения сохраняются;
если расширенные настройки изменены, в Simple появляется небольшая ссылка для перехода к ним.
Раздел Texture Detail работает только при генерации Texture.

Добавь слой **Normal Map** и выбери **Previous** или конкретный источник **Specific** — в том числе
перетаскиванием на Target. Для группы используется её собственная цветная композиция на прозрачном
фоне, без внешней подложки. Источник остаётся редактируемым; конвертация в Drawing и экспорт PSD
растрируют результат.

- **Height Map** строит нормали в касательном пространстве по яркости, R/G/B, альфе или максимуму RGB.
- **Texture** оценивает рельеф по трём диапазонам деталей изображения. Настраивай мелкие, средние
  и крупные детали, радиусы и Light Removal для ослабления широких перепадов освещения.
  Это приближение, а не восстановление геометрии: тени и различия окраски могут стать рельефом.
- Доступны сила, чёрная/белая точки, гамма, инверсия высоты, сглаживание, производные
  Sobel/Scharr/Central Difference, Flip X/Y, края Clamp/Repeat/Mirror и исходная/непрозрачная альфа.
  Ignore Transparent исключает скрытый RGB из сглаживания; при выборе Alpha прозрачность сама
  становится высотой. Радиусы и сила измеряются в пикселях полного холста.
- **Output → Height** показывает восстановленную высоту для настройки; для карты нормалей
  верни **Normal**.
- **Input Space → Color Values** использует отображаемые RGB-значения. **Linear** подходит
  для текстур данных, импортированных без sRGB. На канал Alpha этот выбор не влияет.
- По умолчанию **Encoding → Packed Color** сохраняет значения каналов нормали в PNG/TGA/PSD
  и обычном превью. **Linear Data** оставляет числовые данные для линейного EXR или Texture2D;
  цветное превью будет ярче. Плоская нормаль — `(0.5, 0.5, 1)` в выбранном формате вывода.

Для использования как карты нормалей оставь смешивание Normal, непрозрачность 1, исходный Swizzle
и отключи цветовые FX результата. Обычное смешивание не нормализует векторы повторно; Transform
перемещает изображение, но не переориентирует сами нормали. Экспортированный PNG/TGA импортируй
как **Normal Map**, без преобразования из grayscale. Настройки импорта существующих файлов
автоматически не меняются. См. [параметры API](Documentation~/AgentAPI.md#normal-map-settings).
...
Добавь **Shader Processor**, чтобы обработать уже собранное изображение **ниже его позиции**.
Встроенный редактор Shader FX использует тот же интерфейс `ApplyFX` / `SampleInput` и поддерживает
`#include`. Transform, Swizzle и последовательность модификаторов применяются к этому источнику.
Gaussian Blur остаётся отдельным слоем эффекта с назначаемой целью.

При **Normal** Opacity смешивает исходный и обработанный результат в premultiplied linear space:
при 100% результат заменяет источник вместе с альфой. Остальные режимы смешивают результат с
источником обычным образом. Оба диапазона по умолчанию HDR. Скрытие Processor отключает обработку.

В группе Pass Through он видит и внешний фон; изолированная группа ограничивает обработку своими
дочерними слоями. Отдельное превью, растеризация и использование как цели эффекта собирают нижние
соседние слои на прозрачном фоне. Processor разделяет цепочки обтравки и не служит их участником
или базой. PSD содержит растрированный результат, а исходную структуру — в скрытой папке **Source Layers**.

### Постобработка превью

Кнопка **Post FX** в footer включает постобработку только для просмотра. Стрелочка открывает панель
рядом с настройками и слоями; её закрытие не отключает эффекты. Рисование, пипетка, семплирование
заливки, сохраняемые текстуры и экспорт используют исходную композицию без Post FX.

- **Scene View** наследует включение постэффектов и выбор Volume у активного окна сцены.
- **Game Camera** использует настройки и Volume указанной камеры; пустое поле означает MainCamera.
- **Profile** применяет выбранный Volume Profile с ручными настройками камеры.
- **Solid** задаёт постоянную глубину. **Alpha Height** отображает альфу 1 в Distance, альфу 0 — в
  Distance + Depth Range. **Alpha Mask** отправляет пиксели ниже Threshold на дальнюю плоскость.
  Invert переворачивает альфу перед преобразованием. Источник глубины — исходная альфа композиции,
  а не включённые каналы просмотра.
- **Background → Mode** выбирает **Solid Color** (по умолчанию) либо **Checkerboard** — непрозрачную
  подложку до обработки. Сплошной цвет общий с **User Settings → Post FX Preview**. Шахматка использует
  цвета и размер клеток из настроек прозрачности в User Settings; клетки измеряются в пикселях холста,
  масштабируются с изображением и не меняются из-за Live Quality. Полупрозрачные края смешиваются с
  выбранной подложкой; исходная альфа продолжает управлять глубиной.
- **Link to Zoom** дополнительно связывает дистанцию с зумом. **Animate** обновляет анимированные
  эффекты до 8 раз в секунду; без него превью обновляется по изменениям, настройки Volume проверяются периодически.

Первый необязательный адаптер поддерживает **URP 17.x с Universal Renderer**. Для редактора и
Shader Processor установка URP не нужна. Другие пайплайны и рендереры пока показывают пояснение
и исходное превью. Изолированная непрозрачная поверхность проходит через **активные Renderer Features
выбранного Universal Renderer**, включая Full Screen Pass и SSAO. Глубина и мировые нормали описывают
один альфа-рельеф; SSAO может читать Depth либо Depth Normals. Копии цвета, текстуры глубины и проходы
нормалей планирует URP только по запросам фич, постэффектов или режима рендера. Обычный depth attachment
для непрозрачного рендера всё равно нужен. Выключение Post FX останавливает эти проходы превью и
освобождает ресурсы редактора; общие пулы ресурсов URP могут оставаться в памяти.

Это синтетическая поверхность, а не копия сцены: геометрия сцены, освещение и стек камер не рисуются,
а фильтры объектов у фич могут исключать поверхность. Фичи должны объявлять свои входные текстуры и
поддерживать рендер в текстуру. Проекцию можно наследовать либо переопределить; применяется render scale
пайплайна. TAA, Motion Blur и временная фильтрация AO пропускаются. STP пока не поддерживается —
показывается исходное превью с пояснением. Другие фичи, зависящие от истории движения, настоящей
геометрии, освещения или буферов материалов, не гарантируются. Существующие камеры, профили и
настройки проекта не изменяются. Правки Renderer Features и связанных материалов обновляют превью.
...
| Действие | Результат |
| :--- | :--- |
| **Save** | Обновить текущий документ и результат без диалога. Неактивен до первого сохранения и при отсутствии несохранённых изменений. |
...
Значок предупреждения на **Save As** означает, что у документа ещё нет сохранённого файла.
При закрытии изменённого документа появляется диалог Unity **Save / Discard / Cancel**:
Save запускает Save As, а Cancel отменяет закрытие.
...
**Swizzle** в Layer Settings переназначает четыре выходных канала любого слоя или группы.
В каждом списке доступны `R`, `G`, `B`, `A`, `1-R`, `1-G`, `1-B`, `1-A`, `0`, `1`, `R * A`, `G * A` и `B * A`.
Умножение использует входную альфу до Swizzle независимо от настройки выходного A и не меняет альфу автоматически.
По умолчанию стоит `R G B A`, без изменений. Операция выполняется после FX в линейном
пространстве, перед Color Range и смешиванием, не меняя исходные пиксели.
Изменённый Swizzle изолирует группу; возврат к `R G B A` восстанавливает Pass Through при отсутствии обтравки,
если он выбран у группы. PSD запекает результат, а исходные дочерние слои группы
дополнительно сохраняет в скрытой папке.

**Контекстное меню слоя → Assign Channels** назначает пресеты выделенным слоям сверху вниз по дереву,
независимо от порядка кликов. Это одна команда: пресет автоматически выбирается по числу выделенных слоёв.
Группа считается слоем, явно выделенные дочерние слои считаются отдельно.

- **RGB** (1–3 слоя): направляет `R * A` каждого слоя по порядку в выходные R, G, B, обнуляет остальные RGB и ставит
  альфу в `1`. Все, кроме нижнего выбранного слоя, получают Add; такие группы также переходят в
  Isolated, чтобы Add работал. Наложение нижнего слоя и прозрачность всех слоёв сохраняются.
- **RGBA** (4 слоя): направляет `R * A` каждого слоя по порядку в выходные R, G, B, A, обнуляя остальные каналы.
  Меняется только Swizzle. У RGB-слоёв получается нулевая альфа, поэтому обычное смешивание их
  пропускает: сам по себе этот пресет ещё не собирает готовую четырёхканальную текстуру.

Команда отменяется одним Undo и неактивна при выделении более четырёх слоёв. Слои не сливаются,
не переставляются и не перемещаются между группами; соседние слои и обтравка продолжают влиять на результат.
...
Инструменты: `V` — No Tool · `T` — Transform · `M` — Rectangle Select · `L` — Polygonal Lasso · `B` — Brush · `P` — Pencil · `G` — Fill · `Z` — Zoom.

| Сочетание | Действие |
| :--- | :--- |
| `Ctrl+S` | Save / Save As. |
| `Ctrl+Z` | Отмена. |
| `↑` / `↓` | Выбрать предыдущую / следующую строку слоя; содержимое свёрнутых групп пропускается. |
...
| `Ctrl`-клик / `Shift`-клик по строкам слоёв, вне миниатюр | Переключить выделение слоя / выбрать диапазон. |
| `Ctrl`-клик по миниатюре слоя или стрелке группы | Выделить область по альфе слоя/группы. |
| `Ctrl+C` / `Ctrl+Shift+C` / `Ctrl+V` | Скопировать активный слой / всю композицию / вставить на новый Drawing-слой. |
| `Ctrl+A` / `Ctrl+D` / `Ctrl+Shift+I` | Выделить всё / снять выделение / инвертировать область. |
| `Shift` / `Alt` / `Shift+Alt` с инструментом выделения | Добавить / вычесть / пересечь области. |
| `Enter` / `Backspace` / `Escape` с Polygonal Lasso | Замкнуть / удалить последнюю вершину / отменить незаконченный многоугольник. |
