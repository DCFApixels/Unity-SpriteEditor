...
| Результат | Документ с готовыми Texture2D/Sprite; экспорт в PNG, JPEG, TGA, EXR и Texture2D. |
...
| :--- | :---: | :--- |
| No Tool | `V` | Просмотр без маркеров редактирования и направляющих рисования. Выбран при открытии окна. |
...
Перетаскивание с `СКМ` сдвигает вид; `Escape` отменяет текущее выделение.
...
Кисть и заливку можно выбирать и настраивать без Drawing-слоя; иконки приглушены, если действие недоступно.
...
Тяни за хваталку выбранной строки, чтобы переместить всё выделение. Сбрасывай внутрь группы
...
Вложенные группы работают в режиме **pass-through**: дочерние слои смешиваются прямо с общей композицией.

> [!IMPORTANT]
> У групп есть видимость, но нет собственной прозрачности, режима смешивания, трансформа и FX.
> При этом Outline/SDF могут использовать группу как источник.
...
Цифры задают прозрачность активного слоя: `5` → 50%, затем `7` в пределах 0,6 секунды → 57%.
После паузы `7` → 70%. `0` означает 100%, быстрое `00` — 0%, а `05` — 5%.
Быстрая пара отменяется одним Undo. Меняется прозрачность слоя, а не кисти; для групп это не действует.
...
Встроенные поля и меню строки действуют на эту строку, а не на всё выделение.
...
слой/группу явно (**Specific**). Перетащи хваталку слоя на **Target**, чтобы назначить источник;
...
| **Export** | Записать отдельное сведённое изображение без редактируемых слоёв. |
...
Композиция рассчитывается в **8-битном RGBA**. EXR не добавляет HDR-диапазон и не восстанавливает точность.

<a id="shortcuts"></a>
## Горячие клавиши

Инструменты: `V` — No Tool · `T` — Transform · `B` — Brush · `G` — Fill · `Z` — Zoom.
...
| `Alt`-клик / перетаскивание с `СКМ` при выбранной лупе | Отдалить / сдвинуть вид. |
