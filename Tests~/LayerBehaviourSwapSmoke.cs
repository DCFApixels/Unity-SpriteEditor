...
try
{
    var paint = new DCFApixels.SpriteEditor.DrawingLayerBehaviour { brushColor = Color.red, brushSize = 8, brushHardness = 1 };
    var layer = new DCFApixels.SpriteEditor.Layer(paint) { layerName = "Keep identity" };
    ...
    owned = (Texture2D)typeof(DCFApixels.SpriteEditor.DrawingLayerBehaviour).GetField("pixels", Hidden).GetValue(paint);
    ...
    layer.SetBehaviour(new DCFApixels.SpriteEditor.NoiseLayerBehaviour { seed = 137 });
    ...
    Undo.PerformUndo();
    var restored = doc.layers[0].Behaviour as DCFApixels.SpriteEditor.DrawingLayerBehaviour;
    Check(restored != null && restored.GetPreviewTexture(8).GetPixel(4,4).r > .99f, "Undo restores readable Drawing pixels");
    Undo.PerformRedo();
    Check(doc.layers[0].Behaviour is DCFApixels.SpriteEditor.NoiseLayerBehaviour noise && noise.seed == 137, "Redo restores replacement");
