...
var doc=AssetDatabase.LoadAssetAtPath<DCFApixels.SpriteEditor.TextureCompositor>(Path);
...
DCFApixels.SpriteEditor.Layer Find(string id)=>(DCFApixels.SpriteEditor.Layer)Call(doc,"FindLayer",id);
...
Check(type.Assembly.GetType("DCFApixels.SpriteEditor.CompositionMissingTestBehaviour")==null,"The original type truly disappeared");
...
Check(((DCFApixels.SpriteEditor.OutlineLayerBehaviour)doc.layers[0].Behaviour).TargetLayerId==drawId,"Effect target survives reload");
var fx=doc.layers[1].modifiers[0];
Check(fx is DCFApixels.SpriteEditor.ShaderFX && AssetDatabase.GetAssetPath(fx)==Path,"Embedded FX survives as a subasset");
...
var recovery=type.Assembly.GetType("DCFApixels.SpriteEditor.MissingLayerRecovery");
var statics=System.Reflection.BindingFlags.Static|System.Reflection.BindingFlags.NonPublic;
var record=recovery.GetMethod("FindRecord",statics).Invoke(null,new object[]{doc,typeof(DCFApixels.SpriteEditor.Layer).GetField("behaviourId",Hidden).GetValue(leaf)});
Check(record!=null,"Native recovery record matches stable behaviour ID");
var replacement=new DCFApixels.SpriteEditor.NoiseLayerBehaviour();
...
leaf.SetBehaviour(replacement);group.SetBehaviour(new DCFApixels.SpriteEditor.GroupLayerBehaviour());
...
Undo.PerformRedo();
Check(Find(leafId).Behaviour is DCFApixels.SpriteEditor.NoiseLayerBehaviour noise && noise.seed==613 && Find(groupId).Behaviour is DCFApixels.SpriteEditor.GroupLayerBehaviour,"Recovery Redo restores behaviours");
...
doc=AssetDatabase.LoadAssetAtPath<DCFApixels.SpriteEditor.TextureCompositor>(Path);
Check(Find(leafId).Behaviour is DCFApixels.SpriteEditor.NoiseLayerBehaviour reopened && reopened.seed==613 && Find(leafId).opacity==.35f,"Recovered document saves and reopens");
...
var unsaved=(DCFApixels.SpriteEditor.TextureCompositor)window.GetType().GetField("compositor",Hidden).GetValue(window);
