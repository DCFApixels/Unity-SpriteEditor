...
UnityEngine.Texture2D Pixels(DCFApixels.SpriteEditor.DrawingLayer layer) =>
    (UnityEngine.Texture2D)drawingType.GetProperty("StoredTexture", Hidden).GetValue(layer);
void Point(DCFApixels.SpriteEditor.DrawingLayer layer, UnityEngine.Vector2 uv, object parameters)
{
...
foreach (string shape in new[] { "Circle", "Square", "Diamond" })
{
    var layer = new DCFApixels.SpriteEditor.DrawingLayer { colorRange = DCFApixels.SpriteEditor.LayerColorRange.HDR };
    try
    {
