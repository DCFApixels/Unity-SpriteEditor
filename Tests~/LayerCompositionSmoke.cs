...
var layerType = typeof(DCFApixels.SpriteEditor.Layer);
var behaviourType = typeof(DCFApixels.SpriteEditor.LayerBehaviour);
object Value(object instance, Type type, string name) => type.GetField(name, Hidden).GetValue(instance);
DCFApixels.SpriteEditor.Layer Find(DCFApixels.SpriteEditor.TextureCompositor document, string id) =>
    (DCFApixels.SpriteEditor.Layer)document.GetType().GetMethod("FindLayer", Hidden).Invoke(document, new object[] { id });
try
{
    var child = new DCFApixels.SpriteEditor.Layer(new DCFApixels.SpriteEditor.ColorFillLayerBehaviour { color = Color.red });
    var group = new DCFApixels.SpriteEditor.Layer(new DCFApixels.SpriteEditor.GroupLayerBehaviour()) { layerName = "Keep group", opacity = .7f };
    group.children.Add(child);
    doc.layers.Add(group);
    typeof(DCFApixels.SpriteEditor.TextureCompositor).GetMethod("NormalizeModel", Hidden).Invoke(doc, null);
    string id = child.Id;
    child.layerName = "Keep name";
    child.opacity = .35f;
    child.transform.rotation = 25;
    child.swizzle[0] = DCFApixels.SpriteEditor.SwizzleChannel.B;
    var previous = child.Behaviour;
    var next = new DCFApixels.SpriteEditor.NoiseLayerBehaviour { seed = 913 };
    ...
    bool rejected = false;
    try { new DCFApixels.SpriteEditor.Layer(next); } catch (InvalidOperationException) { rejected = true; }
    Check(rejected, "A behaviour cannot belong to two layers");

    var json = JsonUtility.ToJson(group);
    var copy = JsonUtility.FromJson<DCFApixels.SpriteEditor.Layer>(json);
    Check(copy.Behaviour is DCFApixels.SpriteEditor.GroupLayerBehaviour && copy.children.Count == 1, "JSON restores group structure");
    Check(copy.children[0].Behaviour is DCFApixels.SpriteEditor.NoiseLayerBehaviour noise && noise.seed == 913, "JSON restores polymorphic content");
    ...
    temporary.Add(clone);
    Check(clone.layers[0].children[0].Behaviour is DCFApixels.SpriteEditor.NoiseLayerBehaviour n && n.seed == 913, "Native Unity clone preserves behaviour");
    ...
    rejected = false;
    try { group.SetBehaviour(new DCFApixels.SpriteEditor.FileLayerBehaviour()); } catch (InvalidOperationException) { rejected = true; }
    ...
    group.SetBehaviour(new DCFApixels.SpriteEditor.GroupLayerBehaviour());
    Check(group.children[0] == child && group.opacity == .7f, "Group recovery preserves children and compositing settings");

    var recovery = layerType.Assembly.GetType("DCFApixels.SpriteEditor.MissingLayerRecovery");
    var recordType = recovery.GetNestedType("Record", Hidden);
    var record = Activator.CreateInstance(recordType, true);
    var parse = layerType.Assembly.GetType("DCFApixels.SpriteEditor.MissingLayerData").GetMethod("Parse", Hidden);
    ...
    var blur = new DCFApixels.SpriteEditor.BlurLayerBehaviour();
    ...
    var prepared = new DCFApixels.SpriteEditor.Layer(new DCFApixels.SpriteEditor.DrawingLayerBehaviour());
    var drawing = (DCFApixels.SpriteEditor.DrawingLayerBehaviour)prepared.Behaviour;
