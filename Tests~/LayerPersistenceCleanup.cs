...
foreach (var window in Resources.FindObjectsOfTypeAll<DCFApixels.SpriteEditor.TextureCompositorWindow>())
{
    var doc = (DCFApixels.SpriteEditor.TextureCompositor)typeof(DCFApixels.SpriteEditor.TextureCompositorWindow).GetField("compositor", Hidden).GetValue(window);
...
var fixture = AssetDatabase.LoadAssetAtPath<DCFApixels.SpriteEditor.TextureCompositor>(Path);
bool removed = false;
if (fixture != null)
{
    if (string.IsNullOrEmpty(missingId) || typeof(DCFApixels.SpriteEditor.TextureCompositor)
