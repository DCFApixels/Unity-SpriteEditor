...
for (const hasLayers of [false, true]) for (const dirty of [false, true])
    for (const painting of [null, {}]) for (const transform of [null, { IsDragging: false }, { IsDragging: true }]) {
        assert.equal(closeWarning(hasLayers, () => dirty, painting, transform),
            hasLayers && (dirty || painting !== null || transform?.IsDragging === true));
    }
assert.match(window, /ResolveUnsavedTemporaryDocument\(\)\s*\{\s*PrepareDocumentSave\(\);\s*if \(!HasPreviewLayers \|\| !HasDocumentChanges\(\)\)/);
assert.match(window, /public override void SaveChanges\(\)\s*\{\s*if \(HasPreviewLayers && !SaveAsAsset\(\)\)/);
