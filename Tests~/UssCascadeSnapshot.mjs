...
export function snapshot(source) {
    const rules = [...source.replace(/\/\*[\s\S]*?\*\//g, '').matchAll(/([^{}]+)\{([^{}]*)\}/g)].map(m => ({
        selectors: m[1].split(',').map(s => s.trim().replace(/\s+/g, ' ')
            .replace('.unity-base-popup-field__arrow.sprite-editor-effect-target-arrow', '.sprite-editor-effect-target .unity-base-popup-field__arrow')
            .replace('.unity-base-popup-field__text.sprite-editor-effect-target-text', '.sprite-editor-effect-target .unity-base-popup-field__text')),
    ...
    const theme = rules.find(r => r.selectors.length === 1 && r.selectors[0] === '.sprite-editor-theme');
