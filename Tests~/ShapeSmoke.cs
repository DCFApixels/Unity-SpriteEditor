...
try
{
    ...
    shape.kind = DCFApixels.SpriteEditor.ShapeLayerBehaviour.ShapeKind.Rectangle;
    ...
    var preview = (UnityEngine.Texture2D)typeof(DCFApixels.SpriteEditor.TextureCompositor)
    ...
    layer.enabled = true;
    layer.colorRange = DCFApixels.SpriteEditor.LayerColorRange.HDR;
    layer.blendRange = DCFApixels.SpriteEditor.LayerBlendRange.HDR;
    shape.fillColor = new UnityEngine.Color(2, 0, 0, 1);
    Render(p => Check(p.GetPixel(84, 58).r > 1f, "HDR preserved"));
    var flags = System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static;
    var adjust = typeof(DCFApixels.SpriteEditor.ShapeLayerBehaviour).GetMethod("AdjustCorner", flags);
    ...
    var drag = typeof(DCFApixels.SpriteEditor.TextureCompositorWindow).GetMethod("ShapeDragTransform", flags);
    ...
    foreach (float dx in new[] { -70f, 0f, 70f }) foreach (float dy in new[] { -30f, 0f, 30f })
    {
        ...
        var t = (DCFApixels.SpriteEditor.TextureTransform)drag.Invoke(null, new object[] { start, end, canvas, shape.kind, false, 8f });
        ...
        t = (DCFApixels.SpriteEditor.TextureTransform)drag.Invoke(null, new object[] { start, end, canvas, shape.kind, true, 8f });
        Check(UnityEngine.Mathf.Abs(t.scale.x * canvas.x - t.scale.y * canvas.y) < .0001f, "Shift equal proportions");
        t = (DCFApixels.SpriteEditor.TextureTransform)drag.Invoke(null, new object[] { start, end, canvas, DCFApixels.SpriteEditor.ShapeLayerBehaviour.ShapeKind.Line, true, 8f });
    ...
    Check(DCFApixels.SpriteEditor.SpriteEditorApi.Describe().Contains("shapeDefaults"), "API describes shapes");
    // Exercise the actual partial-update setter without touching a saved asset or another window.
    var setter = typeof(DCFApixels.SpriteEditor.SpriteEditorApi).GetMethod("SetShape", flags);
    ...
    Check(shape.kind == DCFApixels.SpriteEditor.ShapeLayerBehaviour.ShapeKind.Star && shape.sides == 7 && shape.innerRadius == .3f, "API partial update");
    ...
    try
    {
        ...
        var restored = copy.layers[0].Behaviour as DCFApixels.SpriteEditor.ShapeLayerBehaviour;
