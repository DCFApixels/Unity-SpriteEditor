    ...
    public class SDFLayer : Layer
    {
        ...
        public override RenderTexture GetRenderTexture(TextureCompositor compositor, int layerIndex, int width, int height, float scaleMultiplier = 1f)
        {
            ...
            else
            {
                ...
                if (targetLayer != null)
                    inputRT = targetLayer.GetRenderTexture(compositor, targetIdx, width, height, scaleMultiplier);
            }
            ...
            float maxDist;
            if(maxDistanceNormalization > 0)
            {
                maxDist = maxDistanceNormalization;
                if (scaleMultiplier > 0f && !Mathf.Approximately(scaleMultiplier, 1f))
                {
                    maxDist /= scaleMultiplier;
                }
            }
            else
            {
            ...
            // Apply scale multiplier so previews at different resolutions can normalize distances consistently
